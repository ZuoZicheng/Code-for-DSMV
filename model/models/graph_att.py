...
from models.graph_att_layer import GraphSelfAttentionLayer
import time
...
class GAttNet(nn.Module):
    ...
    def forward(self, v_feat, adj_matrix, pos_emb=None):
        ...
        for d in range(self.dir_num):
            ...
            # [batch_size,num_rois, out_feat_dim]
            neighbor_output, aff_softmax = self.neighbor_net[d].forward(
                        self_feat, condensed_adj_matrix, pos_emb,
                        v_biases_neighbors)
            neighbor_emb[d] = neighbor_output
            aff.append(aff_softmax)
