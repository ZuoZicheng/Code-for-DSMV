...
from models.graph_att import GAttNet as GAT
from models.fc import FCNet


def q_expand_v_cat(q, v, mask=True):
    # DSMV uses q_dim=0: no question input to upstream image encoding.
    if q is None:
        return v
...
class ImplicitRelationEncoder(nn.Module):
    ...
    def forward(self, v, position_embedding, q=None):
        """
        Args:
        ...
        for i in range(self.num_steps):
            ...
            imp_v_rel, aff = self.implicit_relation.forward(v_cat_q,
                                                       imp_adj_mat,
                                                       position_embedding)
            affs.append(aff)
            if self.residual_connection:
                imp_v = imp_v + imp_v_rel
            else:
                imp_v = imp_v_rel
        return imp_v, affs[0]
...
class ExplicitRelationEncoder(nn.Module):
    ...
    def forward(self, v, exp_adj_matrix, q=None):
        """
        Args:
            v: [batch_size, num_rois, v_dim]
            q: [batch_size, q_dim]
        ...
        for i in range(self.num_steps):
            ...
            exp_v_rel, aff = self.explicit_relation.forward(v_cat_q, exp_adj_matrix)
            if self.residual_connection:
                exp_v = exp_v + exp_v_rel
            else:
                exp_v = exp_v_rel
        return exp_v, aff
