...
class GraphSelfAttentionLayer(nn.Module):
    def __init__(self, feat_dim, nongt_dim=20, pos_emb_dim=-1,
        ...
        self.linear_out_2 = nn.Linear(self.fc_dim * feat_dim,self.dim[2])
    ...
    def forward(self, roi_feat, adj_matrix,
        ...
        assert (not weighted_aff.isnan().any())
        if position_embedding is not None and self.pos_emb_dim > 0:
            ...
            # position_feat_1, [batch_size,num_rois * nongt_dim, num_heads]
            position_feat_1 = self.pair_pos_fc1(position_embedding_reshape)
            position_feat_1_relu = nn.functional.relu(position_feat_1)

            # aff_weight, [batch_size,num_rois, nongt_dim, num_heads] /// fc_dim = num_heads
            aff_weight = position_feat_1_relu.view(
                (batch_size, -1, nongt_dim, self.fc_dim))

            # aff_weight, [batch_size,num_rois, num_heads, nongt_dim]
            aff_weight = torch.transpose(aff_weight, 2, 3)

            thresh = aff_weight.new_tensor(1e-6)
            # weighted_aff, [batch_size,num_rois, num_heads, nongt_dim]
        ...
        assert (not weighted_aff.isnan().any())
        # aff_softmax, [batch_size, num_rois, num_heads, nongt_dim]
        aff_softmax = nn.functional.softmax(weighted_aff, 3)
        try:
            assert (not aff_softmax.isnan().any())
        except:
            print(weighted_aff)
            assert (not aff_softmax.isnan().any())
        # aff_softmax_reshape, [batch_size, num_rois*num_heads, nongt_dim]
        aff_softmax_reshape = aff_softmax.view((batch_size, -1, nongt_dim))

        # output_t, [batch_size, num_rois * num_heads, feat_dim]
        output_t = torch.matmul(aff_softmax_reshape, v_data)

        assert(not aff_softmax_reshape.isnan().any())
        assert (not output_t.isnan().any())
        # output_t, [batch_size*num_rois, num_heads * feat_dim, 1, 1] /// fc_dim = num_heads
        output_t = output_t.view((-1, self.fc_dim * self.feat_dim, 1, 1))

        linear_out = self.linear_out_2(output_t.reshape(output_t.shape[0], output_t.shape[1]))
        assert(not linear_out.isnan().any())
        output = linear_out.view((batch_size, num_rois, self.dim[2]))
        assert(not output.isnan().any())
        return output, aff_softmax

        # return output, weighted_aff
